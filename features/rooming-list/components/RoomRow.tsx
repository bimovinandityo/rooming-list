    ...
    >
      ...
      <div className="w-44 shrink-0">
        ...
        {room.privateBathroom && (
          <div className="text-[11px] text-gray-400 mt-0.5">Salle de bain privée</div>
        )}
      </div>
      ...
      <div className="w-16 shrink-0">
        ...
        >
          {room.slots.length} lit{room.slots.length > 1 ? "s" : ""}
        </span>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 flex-1 min-w-0 py-1">
        {room.slots.map((slot) =>
          slot.participant ? (
            <div
              key={slot.id}
              className="group flex items-center gap-1.5 bg-gray-100 border border-gray-200 rounded px-2.5 py-1 text-sm text-slate-700 select-none"
            >
